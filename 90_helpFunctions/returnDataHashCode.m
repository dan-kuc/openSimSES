%% returnDataHashCode
% 
% Returns hash code for data input
%
% Input:
...
%   hashing method.
%
% Original script was changed ('uint8' to 'double'), to work with longer 
% data input with lengths > 31536000 are splitted in two parts for 
% processing. The resulting two hex values are sumed up and translated in 
% uint8 or char.
%
...
function hashCode = returnDataHashCode(Data, Opt)
...
try
   ...
   %Engine_2 = java.security.MessageDigest.getInstance(Method); % Für lange Daten.
catch
...
if isFile
   ...
   % Calculate the hash:
   if usetypecastx
      hashCode = typecastx(Engine.digest, 'uint8');
   else
      hashCode = typecast(Engine.digest, 'uint8');
   end
...
elseif usetypecastx  % Array with type, Faster typecastx:
   ...
   if isstruct(Data) % Definieren Größe der Daten
       try
           Size = size(Data.Profile);
       catch
           Size = size(Data.Profile_1);
       end
   else    
       Size = size(Data);
   end
   
   if Size(1,1)<(31536000 + 2)    % Ergänzung für lange Daten. 
       Engine = CoreHash(Data, Engine);
       hashCode   = typecast(Engine.digest, 'uint8');
   else
       if isstruct(Data)    % es gilt nur für struct mit einem Profil sonst Fehler auftreten.
           Data_struct_1 = Data;
           Data_struct_2 = Data;
           Data_struct_1.Profile(1:length(Data_struct_1.Profile)/2) = [];
           Data_struct_2.Profile(fix(length(Data_struct_2.Profile)/2):end) =[];
           
           Engine  = CoreHash(Data_struct_1, Engine);
           digest1 = Engine.digest;
           Engine  = CoreHash(Data_struct_2, Engine);
           digest2 = Engine.digest;
           hashCode    = typecast(digest1+digest2, 'uint8'); 
       else
           Data1   = Data(1:length(Data)/2);
           Data2   = Data(fix(length(Data)/2+1):end);       
           Engine  = CoreHash(Data1, Engine);
           digest1 = Engine.digest;
           Engine  = CoreHash(Data2, Engine);
           digest2 = Engine.digest;
           hashCode    = typecast(digest1+digest2, 'uint8'); 
       end
   end
...
switch OutFormat
   ...
   case 'hex'
      hashCode = sprintf('%.2x', double(hashCode));
   case 'HEX'
      hashCode = sprintf('%.2X', double(hashCode));
   case 'double'
      hashCode = double(reshape(hashCode, 1, []));
   case 'uint8'
      hashCode = reshape(hashCode, 1, []);
   case 'base64'
      hashCode = fBase64_enc(double(hashCode));
   otherwise
...
elseif isnumeric(Data)
   if isreal(Data)
      Engine.update(typecast(Data(:), 'uint8'));%%%%%%%%%%%%%%%% double Fehler???
   else
      Engine.update(typecast(real(Data(:)), 'uint8'));
      Engine.update(typecast(imag(Data(:)), 'uint8'));
   end
